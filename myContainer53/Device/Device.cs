...
namespace myContainer53.Device {
    public class Device {
        ...
        public void ReadDeviceTemp(int page, PortAdapter portAdapter, byte[] deviceAddress, byte[] memory) {

            try {
                logMemoryMap.ReadPageCRC(portAdapter, page, false, memory, 0, null, deviceAddress);
            }
            catch (Exception e) {
                Console.WriteLine(e.Message);
            }
